...
window.addEventListener("keydown", e => {
    ...
    if (num >= 1 && num <= 4) {
        rotationMode = (rotationMode === num) ? 0 : num;
    }
    if (e.key === "z") {
        reverseMode ^= 1;
    }
...
function switchToNext() {
    ...
    if (dragonManager.individuals.length === 1) {
        return;
    }
...
function loop() {
    function frame() {
        ...
        if (drawMode === 1) {
            ...
            // WebGPU モード
            if (dragonManager.container) {
                dragonManager.container.visible = true;
            }
            ...
            // 2D モード(描画重ねる)
            if (dragonManager.container) {
                dragonManager.container.visible = false;
            }
            drawAll();
        }
    }
