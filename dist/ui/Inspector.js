...
const createInspectorGUI = (containerIdIndex) => {
    ...
    if (!topContainer || !contentAreaWrapper || !contentArea) {
        return;
    }
    // --- 下部パラメータエリアのみをクリア ---
        ...
        for (const key in params) {
                    ...
                    const renderList = () => {
                        ...
                        DragonScope.images.forEach((img, idx) => {
                            ...
                            delBtn.onclick = (e) => {
                                ...
                                if (!confirm(`Delete image [index: ${idx}]?`)) {
                                    return;
                                }
            ...
            // --- 6. 数値スライダー ---
                ...
                slider.addEventListener('pointerdown', (e) => {
                    if (sliderLocked || activePointerId)
                        return;
                    e.preventDefault();
                ...
                window.addEventListener('pointermove', (e) => {
                    if (!isSliderDragging || e.pointerId !== activePointerId)
                        return;
