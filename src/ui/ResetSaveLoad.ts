import type { SaveDragonJson } from '../base/type.js';
...
const ResetSaveLoad = {
  sync(id: string, dragon: Dragon, toDragon = true) {
      AllPropSchema_KEYS_excId.forEach((key) => {
      //reset,load処理
        if (toDragon) {
        // NOTE: ループによる動的代入において、左右の key の型同期をTypeScriptが静的解析できないため、一括処理のDRY性を優先して as any を許容
          (dragon as any)[key] = DragonScope.storage[id].saved[key];
      //save,init処理
        } else {
          if (dragon[key] !== undefined) {
        // NOTE: 同上
            DragonScope.storage[id].saved[key] = dragon[key] as any;
          }}});},

reset(id: string, dragon: Dragon){
...
if(currentImgIndex && dragon.resetParts){
  ...
  async save(id: string, dragon: Dragon, blobMaster?: Blob | null, blobPart?: Blob | null) {
  const d = DragonScope.selectedDragon;
    ...
    if (d.name === "Master") {
        ...
        //jsonファイル保存しない場合は終了
        if (!confirm(`want to download JSON Files, too?`)){return;}

  //Master選択時は、followIdとfollowIndexプロパティも含めた全パートのjsonファイル１つを保存（各パートのzip保存は破棄）
    type SaveDragon = Omit<Dragon, "followId"> & {followId: string | null;};
    // mapで中身を SaveDragon[] に変換しながらディープコピーする(followIdを文字列に変換するため)
    const allData: SaveDragon[] = DragonScope.dragons.map(dragon => {
        // ディープコピーを作成
    ...
    const ma = document.getElementById('master-save-link') as HTMLAnchorElement;
    if (ma) {
   ...
   //改めて現時点の情報セーブ
    ...
    const da = document.getElementById('dragon-save-link') as HTMLAnchorElement;
    if (da) {
  ...
  //初期起動時にも使用するため、load関数から独立して定義する。
    ...
    // カテゴリ内の各プロパティ（name, scaleX等）をループ
            Object.keys(PROP_SCHEMA[groupKey]).forEach((key) => {
              let defIdx: number = (PROP_SCHEMA[groupKey] as any)[key].length -1;
            if (loadProp.hasOwnProperty(key)) {
              loadSchema[groupKey][key] = (loadProp as any)[key];
            } else {
              loadSchema[groupKey][key] = (PROP_SCHEMA[groupKey] as any)[key][defIdx];
            }});});
    // 文字列になっているfollowIdのみ解決済みのオブジェクト参照に差し替え
    if (loadSchema.meta) {loadSchema.meta.followId = follower;}
    // インスタンス生成
    ...
    DragonScope.storage[newId] = { current: {}, saved: {} };
      AllPropSchema_KEYS_excId.forEach((key: string) => {
        (DragonScope.storage[newId].saved as any)[key] = (key === "followId") ? follower : (loadProp as any)[key];}
      );});
  DragonScope.dragons.forEach(d => {
    if (d.imgIndex >= DragonScope.images.length) {
        d.imgIndex = DragonScope.images.length - 1;
    ...
    load(id: string, dragon: Dragon, data: SaveDragonJson | SaveDragonJson[]) {
    const d = DragonScope.selectedDragon;
    if(!d) { return; }
    if(d.name === "Master"){
      // this.applyData(data);
      this.applyData(data as SaveDragonJson[]);

    } else if (d.name !== "Master"){
      const singleData = data as SaveDragonJson;
    ...
    // let newName = data.name ? data.name : "";
      if (d && newName){
        ...
        (DragonScope.storage[id].saved as any)[key] = (data as any)[key] ?? (DragonScope.storage[id].current as any)[key] ?? (DragonScope.storage[id].saved as any)[key];
        (DragonScope.storage[id].current as any)[key] = {};});
...
saveBtn.onclick = () => {
  ...
  const d = DragonScope.selectedDragon;
  if (!d) { return; }
  const input = document.getElementById('json-loader') as HTMLInputElement;
  input.onchange = (e) => {
    ...
    reader.onload = (event) => {
        ...
        const data: SaveDragonJson | SaveDragonJson[] = JSON.parse(result);
